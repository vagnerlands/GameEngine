...
#include "Holders/CThreadHolder.h"
#include "OpenGL/CRendererGLFW.h"
#include "OpenGL/ShadowsOGL.h"
#include "Ilumination.h"

#ifdef WIN32
DWORD WINAPI BackgroundLoaderGLFW(LPVOID lpParameter)
{
...
void MainGLFW::StartUp(int argv, char** argc)
{
	// Initialize all Factories
	MutexFactoryWin::Initialize();
	SocketFactoryWin::Initialize();
	...
	if (!Graphics::CRendererGLFW::Create() || !Graphics::IRenderer::mRenderer->InitializeGraphics(1280, 720, false))
	{
		//Graphics::CRendererOGL::Destroy();
		EngineCore::IGame::Destroy();
		return;
	...
	Graphics::IRenderer::mRenderer->IsOperational();
	Graphics::Ilumination::Instance().Initialize(new Graphics::ShadowsOGL);

	// creates a thread for loading resources purpose
	CThreadHolder::instance()->registerThread("thBackgroundLoader", 0x03, BackgroundLoaderGLFW);

	while (Graphics::IRenderer::mRenderer->IsOperational())
	{
		// performs main logic features
		EngineCore::IGame::mGame->Display();
	}
