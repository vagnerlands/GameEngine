...
char list_is_full(ObjParser::list *listo)
{
	return(listo->item_count == listo->current_max_size);
}

void list_grow(ObjParser::list *old_listo)
{
	int i;
	ObjParser::list new_listo;
	
	ObjParser::list_make(&new_listo, old_listo->current_max_size*2, old_listo->growable++);
	
	for(i=0; i<old_listo->current_max_size; i++)
		ObjParser::list_add_item(&new_listo, old_listo->items[i] , old_listo->names[i]);
	
	ObjParser::list_free(old_listo);
...
void ObjParser::list_make(ObjParser::list *listo, int start_size, char growable)
{
...
int ObjParser::list_add_item(ObjParser::list *listo, void *item, char *name)
{
	int name_length;
	char *new_name;
...
void* list_get_item(ObjParser::list *listo, void *item_to_find)
{
	int i = 0;
...
void* list_get_name(ObjParser::list *listo, char *name_to_find)
{
	int i = 0;
...
int ObjParser::list_find(ObjParser::list *listo, char *name_to_find)
{
	int i = 0;
...
void list_delete_item(ObjParser::list *listo, void *item)
{
	...
	{		
		if( listo->items[i] == item )
			ObjParser::list_delete_index(listo, i);
	}
}
...
void ObjParser::list_delete_index(ObjParser::list *listo, int indx)
{
	int j;
...
void ObjParser::list_delete_all(ObjParser::list *listo)
{
	int i;
...
void ObjParser::list_free(ObjParser::list *listo)
{
	ObjParser::list_delete_all(listo);
	free(listo->names);
	free(listo->items);
}
